...
func init() {
	httpcaddyfile.RegisterHandlerDirective("ruller_proxy", parseCaddyfile)
}
...
//
//     ruller_proxy [<matcher>] [<upstreams...>] {
//         # upstreams
//         to <upstreams...>
//
//         # load balancing
	...
	for d.Next() {
		...
		for d.NextBlock(0) {
			...
			case "lb_policy":
				...
				name := d.Val()
				modID := "http.ruller_proxy.selection_policies." + name
			...
			case "transport":
				...
				modID := "http.ruller_proxy.transport." + transportModuleName
				unm, err := caddyfile.UnmarshalModule(d, modID)
				if err != nil {
					return err
				}
