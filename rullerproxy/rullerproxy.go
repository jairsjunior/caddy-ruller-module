...
// ------------|-------------
// `{http.ruller_proxy.upstream.address}` | The full address to the upstream as given in the config
// `{http.ruller_proxy.upstream.hostport}` | The host:port of the upstream
// `{http.ruller_proxy.upstream.host}` | The host of the upstream
// `{http.ruller_proxy.upstream.port}` | The port of the upstream
// `{http.ruller_proxy.upstream.requests}` | The approximate current number of requests to the upstream
// `{http.ruller_proxy.upstream.max_requests}` | The maximum approximate number of requests allowed to the upstream
// `{http.ruller_proxy.upstream.fails}` | The number of recent failed requests to the upstream
// `{http.ruller_proxy.upstream.latency}` | How long it took the proxy upstream to write the response header.
// `{http.ruller_proxy.upstream.duration}` | Time spent proxying to the upstream, including writing response body to client.
// `{http.ruller_proxy.duration}` | Total time spent proxying, including selecting an upstream, retries, and writing response.
type Handler struct {
	// Configures the method of transport for the proxy. A transport
	// is what performs the actual "round trip" to the backend.
	// The default transport is plaintext HTTP.
	TransportRaw json.RawMessage `json:"transport,omitempty" caddy:"namespace=http.ruller_proxy.transport inline_key=protocol"`
	...
	CBRaw json.RawMessage `json:"circuit_breaker,omitempty" caddy:"namespace=http.ruller_proxy.circuit_breakers inline_key=type"`
	...
	// - `{http.ruller_proxy.status_code}` The status code
	// - `{http.ruller_proxy.status_text}` The status text
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	defer func() {
		...
		repl.Set("http.ruller_proxy.duration", time.Since(start))
	}()
	...
	for {
		...
		repl.Set("http.ruller_proxy.upstream.address", dialInfo.String())
		repl.Set("http.ruller_proxy.upstream.hostport", dialInfo.Address)
		repl.Set("http.ruller_proxy.upstream.host", dialInfo.Host)
		repl.Set("http.ruller_proxy.upstream.port", dialInfo.Port)
		repl.Set("http.ruller_proxy.upstream.requests", upstream.Host.NumRequests())
		repl.Set("http.ruller_proxy.upstream.max_requests", upstream.MaxRequests)
		repl.Set("http.ruller_proxy.upstream.fails", upstream.Host.Fails())
...
func (h *Handler) reverseProxy(rw http.ResponseWriter, req *http.Request, repl *caddy.Replacer, di DialInfo, next caddyhttp.Handler) error {
	...
	repl.Set("http.ruller_proxy.upstream.latency", duration)
	...
	for i, rh := range h.HandleResponse {
		...
		res.Body.Close()
		repl.Set("http.ruller_proxy.status_code", res.StatusCode)
		repl.Set("http.ruller_proxy.status_text", res.Status)
	...
	repl.Set("http.ruller_proxy.upstream.duration", duration)
...
type LoadBalancing struct {
	...
	SelectionPolicyRaw json.RawMessage `json:"selection_policy,omitempty" caddy:"namespace=http.ruller_proxy.selection_policies inline_key=policy"`
